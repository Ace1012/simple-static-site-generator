...
router.get("/download", async (req, res) => {
    ...
    for (let key of keys) {
        ...
        const filenames = fs.readdirSync(path.join(path.resolve(), `${root}${key}`));
        filenames.forEach((filename) => {
            ...
            switch (true) {
                case homeRegEx.test(filename):
                    ...
                    const home = fs.readFileSync(path.join(path.resolve(), `${root}home/home.html`));
                    const homeContent = new JSDOM(home).window.document;
                    homeContent.querySelector(".generated-site-controls").remove();
                    homeFolder.file("home.html", homeContent.body.parentElement.outerHTML);
                    break;
                case aboutRegEx.test(filename):
                    console.log(`About found - ${filename}\n`);
                    const about = fs.readFileSync(path.join(path.resolve(), `${root}about/about.html`));
                    const aboutContent = new JSDOM(about).window.document;
                    aboutContent.querySelector(".generated-site-controls").remove();
                    aboutFolder.file("about.html", aboutContent.body.parentElement.outerHTML);
                    break;
                case scriptRegEx.test(filename):
                    ...
                    switch (key) {
                        case "home":
                            script = fs.readFileSync(path.join(path.resolve(), `${root}home/script.js`));
                            homeFolder.file("script.js", script);
                            break;
                        case "about":
                            script = fs.readFileSync(path.join(path.resolve(), `${root}about/script.js`));
                            aboutFolder.file("script.js", script);
                            break;
                        case "articles":
                            script = fs.readFileSync(path.join(path.resolve(), `${root}articles/script.js`));
                            articlesFolder.file("script.js", script);
                            break;
                        case "error":
                            script = fs.readFileSync(path.join(path.resolve(), `${root}error/script.js`));
                ...
                case errorRegEx.test(filename):
                    ...
                    const error = fs.readFileSync(path.join(path.resolve(), `${root}error/${filename}`));
                    errorFolder.file(filename, error);
                    break;
                case articleRegEx.test(filename):
                    console.log(`Article found - ${filename}\n`);
                    const article = fs.readFileSync(path.join(path.resolve(), `${root}articles/${filename}`));
                    const articleContent = new JSDOM(article).window.document;
                    articleContent.querySelector(".generated-site-controls").remove();
                    articlesFolder.file(filename, articleContent.body.parentElement.outerHTML);
                    break;
                case imageRegEx.test(filename):
                    console.log(`Image found - ${filename}\n`);
                    const image = fs.readFileSync(path.join(path.resolve(), `${root}images/${filename}`));
    ...
    for (const key of keys) {
        ...
        if (key === "articles") {
            ...
            for (const article of articles) {
                populateNavBar(article.outPutFilePath, article.populatedTemplate, sidebarLinks, markdown, article.name);
            }
        }
        if (key === "about" || key === "home") {
            ...
            populateNavBar(outPutFilePath, populatedTemplate, key === "about" ? sidebarLinks : homeSidebarLinks, markdown, markdown[key].name);
        }
    }
}
/**
 ...
 * @param articles
 */
async function createSidebarLinks(homeSidebarLinks, sidebarLinks, articles) {
    for (const article of articles) {
 ...
 * @param filename
 */
async function populateNavBar(outPutFilePath, populatedTemplate, sidebarLinks, markdown, filename) {
